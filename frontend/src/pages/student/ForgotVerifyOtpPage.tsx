  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success && res.data?.reset_token) {
        setSuccessMsg("Xác thực mã OTP thành công! Chuyển đến trang đặt lại mật khẩu.");
        const token = res.data.reset_token;
        setTimeout(() => {
          navigate(`/forgot-password/reset?email=${encodeURIComponent(email)}&token=${encodeURIComponent(token)}`);
        }, 1500);
      } else {
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-tr from-blue-100 via-white to-blue-50 px-4">
      <div className="w-full max-w-md rounded-2xl bg-white p-8 shadow-xl border border-gray-100">
        <div className="text-center">
          <h2 className="text-3xl font-extrabold text-gray-900 tracking-tight">Xác Thực OTP Quên Mật Khẩu</h2>
