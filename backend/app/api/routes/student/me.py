...
async def get_profile(current_student: dict = Depends(get_current_student)):
    return {"success": True, "data": format_student_profile(current_student)}


@router.patch("")
async def update_profile(
    payload: StudentProfileUpdateRequest, current_student: dict = Depends(get_current_student)
):
    db = get_db()
    now = datetime.utcnow()
...
async def change_password(
    ...
    return {"success": True, "data": {"message": "Đổi mật khẩu thành công."}}
