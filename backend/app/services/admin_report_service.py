...
async def get_report_detail(report_id: str) -> dict:
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail={"success": False, "error": {"code": "REPORT_NOT_FOUND", "message": "Không tìm thấy báo cáo.", "details": []}},
        )
    ...
    if report["target_type"] == "place" and report.get("target_place_id"):
        ...
        if place:
            target_preview = {"type": "place", "public_id": place["public_id"], "name": place["name"], "status": place["status"]}
        ...
        if comment:
            target_preview = {"type": "comment", "id": str(comment["_id"]), "content": comment["content"], "status": comment["status"]}
    ...
    return {
        ...
        "target_place_id": str(report["target_place_id"]) if report.get("target_place_id") else None,
        "target_comment_id": str(report["target_comment_id"]) if report.get("target_comment_id") else None,
...
async def update_report(report_id: str, new_status: str, admin_note: str | None, admin_id: ObjectId, ip_address: str) -> dict:
    db = get_db()
    try:
        oid = ObjectId(report_id)
    except Exception:
...
async def execute_action(report_id: str, action: str, reason: str, admin_id: ObjectId, ip_address: str) -> dict:
    db = get_db()
    try:
        oid = ObjectId(report_id)
    except Exception:
    ...
    if action == "hide_place" and report.get("place_public_id"):
        await admin_place_service.hide_place(report["place_public_id"], reason, admin_id, ip_address)
        ...
        await db["comments"].update_one(
            ...
            {"$set": {"status": "deleted", "admin_delete_reason": reason, "deleted_at": now, "updated_at": now}},
        )
        await audit_service.log_event(
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail={"success": False, "error": {"code": "REPORT_INVALID_STATUS", "message": "Hành động không hợp lệ.", "details": []}},
        )
