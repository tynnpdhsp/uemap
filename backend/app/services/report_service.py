...
status_label_map = {"new": "mới", "in_progress": "đang xử lý", "resolved": "đã xử lý"}


async def create_report(
    student_id: ObjectId, payload: ReportCreateRequest, ip_address: str
) -> dict:
    db = get_db()
    ...
    counter = await db["report_counters"].find_one_and_update(
        {"_id": today_str}, {"$inc": {"seq": 1}}, upsert=True, return_document=True
    )
    seq_num = counter["seq"]
