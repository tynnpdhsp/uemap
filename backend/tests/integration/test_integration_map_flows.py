...
async def clean_map_integration_db():
    ...
    await db["students"].delete_many({"email": {"$in": ["4901104199@student.hcmue.edu.vn", "4901104188@student.hcmue.edu.vn"]}})
...
async def test_map_and_social_integration_flow(mock_media_minio, mock_upload_minio):
    async with api_client() as client:
        ...
        token_locked = await session_service.create_session(student_locked_id, "127.0.0.1", "pytest")
        ...
        res = await client.patch(f"/api/my/places/{public_id_pub}", json=payload_pub, headers=headers_b)
        ...
        res = await client.patch(
            ...
            json={"content": "Phá hoại bình luận"},
            headers=headers_b
        )
        ...
        res = await client.get(f"/api/media/places/{public_id_draft}/image2.webp", headers=headers_b)
